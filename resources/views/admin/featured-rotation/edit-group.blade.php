<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h2 class="text-2xl font-bold text-gray-800" style="font-family: 'Playfair Display', serif;">Edit Product Group</h2>
                    <p class="text-sm text-gray-500">Update {{ $group->name }}</p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg p-6">
        <form action="{{ route('admin.featured-rotation.groups.update', $group->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $group->name) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent" required>
                    @error('name')
                ...
                <div>
                    ...
                    <input type="number" name="order" value="{{ old('order', $group->order) }}" min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent" required>
                ...
                <div>
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $group->is_active) ? 'checked' : '' }} class="w-5 h-5 text-amber-600 border-gray-300 rounded focus:ring-amber-500">
                ...
                <div>
                    ...
                    <div class="border border-gray-300 rounded-lg p-4 max-h-96 overflow-y-auto">
                        ...
                        <div id="productList" class="space-y-2">
                            @php
                                $selectedProductIds = $group->items->pluck('product_id')->toArray();
                            @endphp
                            @foreach($products as $product)
                                <label class="flex items-center p-3 hover:bg-gray-50 rounded-lg cursor-pointer product-item">
                                    <input type="checkbox" name="product_ids[]" value="{{ $product->id }}" {{ in_array($product->id, $selectedProductIds) ? 'checked' : '' }} class="w-5 h-5 text-amber-600 border-gray-300 rounded focus:ring-amber-500">
                ...
                <div class="flex items-center justify-end space-x-3 pt-4 border-t">
                    ...
                    <button type="submit" class="inline-flex items-center px-6 py-2 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-700 hover:to-amber-800 text-white font-semibold rounded-lg shadow-lg transition">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
