                    ...
                    @forelse($carousels as $carousel)
                        <div class="row g-0 header-carousel-item align-items-center">
                            <div class="col-xl-6 carousel-img wow fadeInLeft" data-wow-delay="0.1s">
                                <img src="{{ asset($carousel->image) }}" class="img-fluid w-100" alt="{{ $carousel->title }}">
                            </div>
                            <div class="col-xl-6 carousel-content p-4">
                                <h4 class="text-uppercase fw-bold mb-4 wow fadeInRight" data-wow-delay="0.1s"
                                    style="letter-spacing: 3px;">{{ $carousel->title }}</h4>
                                <h1 class="display-3 text-capitalize mb-4 wow fadeInRight" data-wow-delay="0.3s">{{ $carousel->description }}</h1>
                                @if($carousel->subtitle)
                                    <p class="text-dark wow fadeInRight" data-wow-delay="0.5s">{{ $carousel->subtitle }}</p>
                                @endif
                                @if($carousel->button_text && $carousel->button_link)
                                    <a class="btn btn-primary rounded-pill py-3 px-5 wow fadeInRight" data-wow-delay="0.7s"
                                        href="{{ $carousel->button_link }}">{{ $carousel->button_text }}</a>
                                @endif
                            </div>
                        </div>
                    @empty
